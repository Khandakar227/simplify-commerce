      ...
      <nav className="w-full sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-green-200 shadow-sm flex items-center justify-between px-8 py-3">
        ...
        <div className="flex-1 flex justify-center">
          <div className="flex gap-2">
            <span className="text-green-700 font-semibold px-4 py-2 rounded-lg bg-green-50">Add Product</span>
            <span className="text-green-700 font-semibold px-4 py-2 rounded-lg hover:bg-green-100 transition cursor-pointer">Dashboard</span>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center py-8 px-2">
        <div className="w-full max-w-3xl mx-auto">
          <Card className="bg-amber-50 shadow-xl rounded-xl border border-amber-200 p-6">
            <CardHeader>
              <CardTitle className="font-extrabold text-3xl text-green-700 py-4 text-center">
                ADD A NEW PRODUCT
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="py-5 px-4">
                <form onSubmit={onAddProduct}>
                  <div className="flex gap-4 items-start justify-center py-4 flex-col">
                    <Label className="text-green-700"><b>Name</b></Label>
                    <Input name="name" type="text" className="w-full"/>
                  </div>
                  <div className="flex gap-4 items-start justify-center py-4 flex-col">
                    <Label className="text-green-700"><b>Product Image</b></Label>
                    <Input name="images" type="file" className="w-full cursor-pointer bg-white bg-opacity-15"/>
                  </div>
                  <div>
                    <Label className="text-green-700"><b>Category</b></Label>
                    <CategoryInput selectedCategory={category} onSelectCategory={selectCategory} />
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="flex gap-4 items-start justify-center py-4 flex-col">
                      <Label className="text-green-700"><b>Price</b></Label>
                      <Input name="price" type="number" min="0" className="w-full"/>
                    </div>
                    <div className="flex gap-4 items-start justify-center py-4 flex-col">
                      <Label className="text-green-700"><b>Stock</b></Label>
                      <Input name="stock" type="number" min="0" className="w-full"/>
                    </div>
                  </div>
                  <div className="flex gap-4 items-start justify-center py-4 flex-col">
                    <Label className="text-green-700"><b>Description</b></Label>
                    <Tiptap onContentChange={setDescription}/>
                  </div>
                  <Button type="submit" className="bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow transition w-full mt-4">{ loading ? "Adding..." : "Add Product"}</Button>
                </form>
              </div>
            </CardContent>
          </Card>
